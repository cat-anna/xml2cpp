...
function ContainerType:WriteTable(block)
	local element = self.element_type
	
	block:MakeAlias(self:LocalName(), "std::vector<" .. element:GlobalName() .. ">")
	
	block:BeginBlockLine { "inline bool ", self:LocalName(), "_Write(pugi::xml_node in_node, const ", self:LocalName(), " &value, const char* name) {", }
	...
	block:BeginBlockLine { "inline bool ", self:LocalName(), "_SetDefault(", self:LocalName(), " &value) {", }
		block:Line "value.clear();"
		block:Line "return true;" 
	block:EndBlockLine "}"
end
